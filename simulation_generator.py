...
parser.add_argument("-idfile",help="Name of the file or path to identift the file",type=int,default='0')
...
#            rates_matrix_par[number_levels_par-1-i,7]=1/(1+math.exp(epsilon*(i)))
            rates_matrix_par[number_levels_par-1-i,7]=1/(1.6)**i
#        rates_matrix_par[16,7]=0.1
        rates_matrix_par[number_levels_par-1,7]=6.0
...
#    reduced_rates[17:number_levels,3]=reduced_rates[17:number_levels,1]
    for i in range(3,number_levels_par):
        reduced_rates[number_levels_par-1-i,3]=reduced_rates[number_levels_par-1-i,1]/10
    for i in range(0,3):
        reduced_rates[number_levels_par-1-i,3]=reduced_rates[number_levels_par-1-i,1]
...
#    print(sum_rates[:number_levels-2])
    ...
    for i in range(3,number_levels_par):
        last_block[number_levels_par-1-i,3]=last_block[number_levels_par-1-i,1]/10
    for i in range(0,3):
        last_block[number_levels_par-1-i,3]=last_block[number_levels_par-1-i,1]
...
alpha_string='{:0.3f}'.format(alpha)
beta_string='{:0.2f}'.format(beta)
leak_function=0
fitness_s=0.15 #fitness of the mutant cells
directory_name='final_'+'alpha_'+str(alpha_string)+'_beta_'+str(beta_string)+'_mu_'+str(mu_string)+'_test_'+str(epsilon_string)+'/'

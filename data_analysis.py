...
#    ax1.fill_between(time,normal_value_low,normal_value_up,label='Normal region',alpha=0.3,color='salmon')
    ax1.plot(time,normal_value_up,label='Maximum healthy WBC count',color=color,linestyle=':')
    ax1.plot(time,common_value_diagnosis,label='Usual WBC count at diagnosis',color=color)
    ax1.scatter(time,total_wbc, color=color,label='WBC count')
    ax1.legend(loc=2, borderaxespad=0, fontsize=20)
...
#    col = ['violet','yellow','seagreen','orchid','cyan','goldenrod','darkseagreen','darkorange']
    ...
    ax2.legend(loc=6, borderaxespad=0, fontsize=20)
    fig.tight_layout()
#    plt.show()

...
rate_number=5 #Number of available events in the simulation, in this version it contains scd, scdif, adif, cell death and mutation
gamma=args.gamma
p=args.p
p_stem_cell=0.5
sim_time=args.t
...
def perform_event_stabilize(event_to_perform_par,level_to_perform_par,number_levels_par):
    ...
    if event_to_perform_par==0 and (level_to_perform_par%number_levels_par)!=0:#perform scd
        c[level_to_perform_par]=c[level_to_perform_par]+1
    elif event_to_perform_par==1 and (level_to_perform_par%number_levels_par)!=number_levels_par-1 and (level_to_perform_par%number_levels_par)!=0:#perform scdif in a progenitor level
        c[level_to_perform_par]=c[level_to_perform_par]-1
        c[level_to_perform_par+1]=c[level_to_perform_par+1]+2
    ...
    elif (level_to_perform_par%number_levels_par)==0 and c[level_to_perform_par]>c_set[level_to_perform_par]:
        c[0]=c[0]-1
        c[1]=c[1]+2
    elif (level_to_perform_par%number_levels_par)==0 and c[level_to_perform_par]<c_set[level_to_perform_par]:
        c[0]=c[0]+1
...
def create_new_mutated_levels(c_par,number_levels_par,level_to_perform_par):
    ...
    if number_of_mutations==0:
        initial_rates[number_levels_par:,:]=initial_rates[0:number_levels_par,:]
    else:
...
#    print(level_to_perform,event_to_perform)

...
c_set=np.zeros((number_levels),dtype=int)
c_set[0]=400
for i in range(1,number_levels):
    c_set[i]=np.rint(2.0**i*c_set[0])
#for i in range(20,number_levels):
#    c_set[i]=np.rint(c_set[19])
...
def construct_rates(rates_matrix_par,number_levels_par,gamma_par,p_par,p_stem_cell_par,c_set_par):
    ...
    def calculate_delta_mod():
        for i in range(number_levels_par-3,mitotic_pool,-1):
            rates_matrix_par[i,0]=rates_matrix_par[i+1,0]/gamma_par
    ...
    set_rscdif()
    set_radif()
    print((rates_matrix_par[0,0]*365)/c_set_par[0])
...
#    print(fitness_term)
    ...
    bm_index=np.empty((marrow_limits+1,),dtype=object)
    number_of_cells=np.empty((marrow_limits+1,),dtype=object)
    for i in range(0,marrow_limits+1):
        bm_index[i]=[k for k in range(i,len(c_par),number_levels)]
        number_of_cells[i]=np.sum(c[bm_index[i]])
    bm_cells_set=np.sum(c_set[0:mitotic_pool+1])
    bm_cells=np.sum(number_of_cells)
    stem_index=[i for i in range(0,len(c_par),number_levels)] #Index of stem cell levels
    prog_scd_index=[0 for i in range(0,len(c_par),number_levels)]  #Index od scd rates
    prog_scdif_index=[1 for i in range(0,len(c_par),number_levels)] #Index of scdif rates
    ...
    if number_stem_cells>c_set[0]: #Ignoring scd when we have excess of stem cells and keep constant the rate of stem cell
        local_rates[stem_index,prog_scd_index]=prog_scd_index
        local_rates[stem_index,prog_scdif_index]=local_rates[stem_index,prog_scdif_index]*2
        ...
        local_rates[stem_index,prog_scdif_index]=prog_scd_index
        local_rates[stem_index,prog_scd_index]=local_rates[stem_index,prog_scd_index]*2-fitness_term[stem_index]
#    for i in range(1,marrow_limits+1):
#        if bm_cells>(10*bm_cells_set): #Ignoring scd when we have excess of stem cells and keep constant the rate of stem cell
#            local_rates[bm_index[i],prog_scd_index]=prog_scd_index
#        else:
#            1
...
def perform_event_left(event_to_perform_par,level_to_perform_par,number_levels_par,l_mutations,num_cell_par):
    ...
    global initial_rates
    global count_stem
    if level_to_perform_par%number_levels_par==0:
...
#            print(num_mut_right,num_cells_right)
    ...
    #        print(num_mut_right,num_cells_right)
            ...
            for l in range(np.size(num_mut_right)):
                perform_event_right(event_number_list[1][i],event_number_list[0][i],number_levels_par,num_mut_right[l],num_cells_right[l])            
    data=np.concatenate((t,c),axis=0).reshape(1,c.shape[0]+1)

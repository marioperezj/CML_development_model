...
final_output_events=int((3.5e11)/4)
gamma=2.5
...
def construct_rates(rates_matrix_par,number_levels_par,gamma_par,p_par,p_stem_cell_par,c_set_par):
    ...
    def set_p_values():
        for i in range(1,prog_levels,1):
            rates_matrix_par[i,1]=p_prog
        for i in range(prog_levels,number_levels_par-1,1):
...
#    print(fitness_term)
    ...
    prog_index=np.empty((prog_levels+1,),dtype=object)
    number_of_cells=np.empty((prog_levels+1,),dtype=object)
    for i in range(0,prog_levels+1):
        prog_index[i]=[k for k in range(i,len(c_par),number_levels)]
        number_of_cells[i]=np.sum(c[prog_index[i]])
#    print(number_of_cells)
    ...
    for i in range(1,prog_levels+1):
        if number_of_cells[i]>c_set[i]: #Ignoring scd when we have excess of stem cells and keep constant the rate of stem cell
            local_rates[prog_index[i],prog_scd_index]=prog_scd_index
        else:
            1
#            local_rates[stem_index[i],scd_index]=local_rates[stem_index,scd_index]*2-fitness_term[stem_index]
